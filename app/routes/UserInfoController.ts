...
import { UsesrInfoService } from "../services/UsesrInfoService";
import { RawQuery } from "../common/RawQuery";

export class UserInfoController {
  private router: Router = Router();
  private service = new UsesrInfoService();
  ...
  getRouter(): Router {
    ...
    this.router.post("/changepassword", async (request: Request, response: Response) => {
      try {
        let reqData: any;
        let result: any = null;
        this.service.sessionInfo = request.body.sessionInfo;
        ...
        reqData = request.body ? request.body.data : {};
        if (await App.ValildateUserAccess(this.service.sessionInfo, this.moduleName(), Props.ACCESS_WRITE)) {
          result = await this.service.changePassword(reqData);
        } else {
          throw this.service.sessionInfo ? this.service.sessionInfo : { message: Props.TOKEN_MESSAGE };
        }
        response.send({ status: 1, data: result });
