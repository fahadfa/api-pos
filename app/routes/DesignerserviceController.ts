...
import { DesignerserviceService } from "../services/DesignerserviceService";

export class DesignerserviceController {
  private router: Router = Router();
  private service = new DesignerserviceService();
  ...
  getRouter(): Router {
    ...
    this.router.get("/designerservicelist", async (request: Request, response: Response) => {
      try {
        let reqData: any;
        let result: any = null;
        this.service.sessionInfo = request.body.sessionInfo;
        ...
        reqData = request.query ? request.query : {};
        if (await App.ValildateUserAccess(this.service.sessionInfo, this.moduleName(), Props.ACCESS_READ)) {
          result = await this.service.designerServiceList(reqData);
        } else {
          throw this.service.sessionInfo ? this.service.sessionInfo : { message: Props.TOKEN_MESSAGE };
        }
        response.send({ status: 1, data: result });
