                ...
                switch (_a.label) {
                    ...
                    case 3:
                        ...
                        if (params.status != "ALL") {
                            if (params.status == "APPROVEDBYRA") {
                                query += " and s.status in ('APPROVEDBYRA') ";
                            }
                            else if (params.status == "APPROVEDBYRM") {
                                query += " and s.status in ('APPROVEDBYRM') ";
                            }
                            else if (params.status == "CREATED" || params.status == "SAVED") {
                                query += " and s.status in ('CREATED', 'SAVED) ";
                            }
                            else if (params.status == "POSTED") {
                                query += " and s.status in ('POSTED','PAID') ";
                            }
                            else if (params.status == "PENDINGRMAPPROVAL") {
                                query += " and s.status in ('PENDINGRMAPPROVAL','PENDINGRMAPPROVAL') ";
                            }
                        }
