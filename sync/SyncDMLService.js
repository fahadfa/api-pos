...
var SyncDMLService = /** @class */ (function() {
      ...
      return __generator(this, function(_a) {
        switch (_a.label) {
          case 0:
            ...
            sql =
              "SELECT to_char (now(), 'YYYY-MM-DD\"T\"HH24:MI:SS') as utc_date";
            return [
              4 /*yield*/,
              SyncServiceHelper_1.SyncServiceHelper.ExecuteQuery(
                stageDbConfig,
                sql
              )
            ];
          ...
          case 3:
            ...
            if (!syncResults) return [2 /*return*/, Promise.resolve("")];
            if (!(syncResults.source_id != syncResults.target_id))
              return [3 /*break*/, 5];
            sourceDB =
              syncResults.source_id == STAGING_ID
                ? stageDbConfig
                : localDbConfig;
            targetDB =
              syncResults.target_id == STORE_ID ? localDbConfig : stageDbConfig;
            return [
              4 /*yield*/,
              this.syncDb(sourceDB, targetDB, syncResults, currentTime)
            ];
          case 4:
            _a.sent();
            _a.label = 5;
  ...
  SyncDMLService.prototype.syncDb = function(
    sourceDb,
    targetDb,
    sync,
    currentTime
      ...
      return __generator(this, function(_a) {
        switch (_a.label) {
                ...
                switch (_a.label) {
                  ...
                  case 1:
                    ...
                    if (!(soruceRes && soruceRes.rows.length != 0))
                      return [3 /*break*/, 11];
                    primaryKeys = soruceRes.rows.map(function(ele) {
                      return ele[sync.map_pk];
                    });
                    return [
                      4 /*yield*/,
                      SyncServiceHelper_1.SyncServiceHelper.ChackAvalibleQuery(
                        sync.map_table,
                        soruceRes.metaData,
                        primaryKeys,
                        sync.map_pk
                  ...
                  case 3:
                    res = _a.sent();
                    rowsAvalible_1 = res.rows.map(function(ele) {
                      return ele[sync.map_pk];
                    });
                    rowsNotAvalible = primaryKeys.filter(function(ele) {
                      return rowsAvalible_1.indexOf(ele) < 0;
                    });
                    return [
                      4 /*yield*/,
                      SyncServiceHelper_1.SyncServiceHelper.MetadataTable(
                        targetDb,
                        sync.map_table
                      )
                    ];
                  case 4:
                    ...
                    Log_1.slog.info("\n\n************* ***** *************");
                    Log_1.slog.debug(
                      "\t\tUpdate Records: " + rowsAvalible_1.length
                    );
                    Log_1.slog.debug(
                      "\t\tInsert Records: " + rowsNotAvalible.length
                    );
                    Log_1.slog.info("************* ***** *************");
                    if (!(rowsAvalible_1 && rowsAvalible_1.length > 0))
                      return [3 /*break*/, 6];
                    return [
                      4 /*yield*/,
                      SyncServiceHelper_1.SyncServiceHelper.PrepareQuery(
                        sync.map_table,
                        metaDataTable,
                        soruceRes.rows,
                        rowsAvalible_1,
                  ...
                  case 8:
                    ...
                    return [
                      4 /*yield*/,
                      SyncServiceHelper_1.SyncServiceHelper.BatchQuery(
                        targetDb,
                        batchSql
                      )
                    ];
                  ...
                  case 10:
                    ...
                    if (soruceRes.rows.length < this_1.limitData) {
                      Log_1.slog.debug("Batch completed...");
                      isChunkEnd = true;
                    }
                    return [3 /*break*/, 12];
                  case 11:
          ...
          case 3:
            ...
            return [3 /*break*/, 2];
          case 4:
            if (!(isTableUpdated == true)) return [3 /*break*/, 6];
            updateQuery =
              "update sync_table set last_update = '" +
          ...
          case 7:
            ...
            if (err_1 == Props_1.Props.RECORD_NOT_FOUND) {
              ...
              updateQuery =
                "update sync_table set updated_on = '" +
                currentTime +
                "'  where id='" +
                sync.id +
                "'";
  ...
  SyncDMLService.prototype.buildDDLQuery = function(sync, offset) {
    var sql =
      ...
      sync.sync_column +
      " >= '" +
      new Date(sync.last_update).toISOString() +
      "'\n                 offset " +
      offset +
      " limit " +
      this.limitData;
